...
```
```
Этот код определяет новый класс администратора под названием CustomUserAdmin, который наследуется от UserAdmin, чтобы получить некоторые функции по умолчанию. Класс CustomUserAdmin переопределяет некоторые настройки и методы по умолчанию для добавления запрошенных вами новых функций.

Атрибут list_display задает поля, которые отображаются в виде списка. Атрибут search_fields задает поля, по которым выполняется поиск при использовании панели поиска. Атрибут list_filter задает поля, которые можно использовать для фильтрации представления списка.

Атрибут exclude скрывает поле пароля. Атрибут readonly_fields задает поля, которые нельзя редактировать. Атрибут fieldsets задает порядок и группировку полей в режиме редактирования.

Метод get_fieldsets позволяет пользователям создавать новые объекты CustomUser, заполняя только обязательные поля.

Метод get_readonly_fields делает поле электронной почты доступным только для чтения при редактировании существующего объекта CustomUser.

Метод save_model переопределяется для установки пароля с помощью set_password(), когда пользователь сохраняет форму и изменяет пароль.
```
